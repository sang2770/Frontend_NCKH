...
const ShowNotification = ({ data, onClickHide, datadelete, setDelete }) => {
      ...
      idTB &&
        ...
        Client.put(
          "/notification-management/update-notification/" +
            idTB +
            "?NoiDungTB=" +
            FilterNoiDungTB.current.value +
            "&TieuDeTB=" +
            FilterTieuDeTB.current.value
        )
          .then((response) => {
  ...
  const UpdateFileName = (idDelete) => {
    idDelete &&
      Client.put(
        "/notification-management/update-notification-file/" + idDelete
      )
        .then((response) => {
          if (response.data.status === "Success") {
            // alert("Success!");
  ...
  const deleteFile = (idDelete) => {
    if (window.confirm("Bạn có muốn xóa file này không?") === true) {
      idDelete &&
        Client.delete(
          "/notification-management/delete-notification-file/" + idDelete
        )
          .then((response) => {
            if (response.data.status === "Success") {
  ...
  const SubmitFile = async (form, ResetForm) => {
    try {
      const result = await Client.post(
        "/notification-management/post-notification-file/" + idTB,
        form,
        {
          headers: {
                    ...
                    <form onSubmit={SubmitFormm}>
                      ...
                      <div className={Style.Content_file}>
                        ...
                        <input
                          type="file"
                          name="file"
                          className={style.Content_input_file}
                        ></input>
                        <button
